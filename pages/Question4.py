...
def main():
    ...
    with col2:
        st.title("You are assigned with difficult project with very short deadline, what will your strategy?")
        text_input = st.text_input("Your Answer")

    # Button to move to Question 2
    with col2:
        if st.button("Move to Question 5"):
            common_list.append(text_input)
            session_state.common_list = common_list
            st.switch_page('pages/Question5.py')
